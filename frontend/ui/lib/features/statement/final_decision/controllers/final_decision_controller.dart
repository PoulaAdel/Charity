...
class FinalDecisionController extends GetxController {
  ...
  Rx<Opinion?> opinion = Rx<Opinion?>(null);
  Rx<Suggestion?> suggestion = Rx<Suggestion?>(null);
  ...
  Future<void> fetchOpinion() async {
    if (selectedStatement.value != null) {
      try {
        isLoading.value = true;
        var response = await _api.get('opinions', queryParameters: {
          'statement': selectedStatement.value!.pk.toString(),
        });
        if (response.isNotEmpty) {
          final opinionObj = Opinion.fromJson(response.first);
          if (opinionObj.content.isNotEmpty) {
            opinion.value = opinionObj;
          } else {
            opinion.value = null;
          }
        } else {
        ...
        Get.snackbar('Error', 'Failed to fetch opinion data: $e',
            snackPosition: SnackPosition.BOTTOM);
      } finally {
        isLoading.value = false;
      }
  ...
  Future<void> fetchSuggestion() async {
    if (selectedStatement.value != null) {
      try {
        isLoading.value = true;
        var response = await _api.get('suggestions', queryParameters: {
          'statement': selectedStatement.value!.pk.toString(),
        });
        if (response.isNotEmpty) {
          final suggestionObj = Suggestion.fromJson(response.first);
          if (suggestionObj.content.isNotEmpty) {
            suggestion.value = suggestionObj;
          } else {
        ...
        Get.snackbar('Error', 'Failed to fetch suggestion data: $e',
            snackPosition: SnackPosition.BOTTOM);
      } finally {
        isLoading.value = false;
      }
