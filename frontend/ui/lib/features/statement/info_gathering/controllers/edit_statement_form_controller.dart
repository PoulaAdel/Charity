...
class EditStatementFormController extends GetxController {
  ...
  List parseObjectList(List<dynamic> list) {
    ...
    switch (modelType.value) {
      case 'spiritual':
        dynamicTypeList = list.map((item) => Spiritual.fromJson(item)).toList();
        break;
      case 'economical':
        dynamicTypeList =
            list.map((item) => Economical.fromJson(item)).toList();
        break;
      case 'social':
        dynamicTypeList = list.map((item) => Social.fromJson(item)).toList();
        break;
      case 'residential':
        dynamicTypeList =
            list.map((item) => Residential.fromJson(item)).toList();
        break;
      default:
        dynamicTypeList = [];
    }
